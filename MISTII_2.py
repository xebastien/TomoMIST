...
#    Solving system for each pixel 
    ...
    print("Data reformatted in memmory")
    #Calculation of G
    ...
    # prepare for regularisation of integration
    ...
    thickness = -Lambda/(4*np.pi)*np.log(ifft2(ifftshift(fftshift(fft2(G))/denom * beta)))
    phi = thickness.real
...
def processProjectionMISTII_2(Is,Ir,expParam):
    """
    ...
    thickness, Deff_xx,Deff_yy,Deff_xy = MISTII_2(Is,Ir,expParam,nbImages)
    ...
    return {'phi': thickness, 'Deff_xx': Deff_xx, 'Deff_yy': Deff_yy, 'Deff_xy': Deff_xy, 'ColoredDeff': colouredImage}
